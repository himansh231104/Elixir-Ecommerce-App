...
import './style.css';
import { ShoppingBag, ChevronRight, Truck, CreditCard, Check, Wallet } from 'lucide-react';
      ...
      <div className="order-page-container">
        ...
        <div className="order-page-steps">
          {steps.map((step) => (
            <div 
              key={step.id} 
              className={`order-page-step ${activeStep === step.id ? 'active' : ''} ${activeStep > step.id ? 'completed' : ''}`}
            ...
            >
              ...
              {step.id !== steps.length && <ChevronRight className="order-page-step-arrow" />}
            </div>
          ))}
        </div>
              ...
              <div className="order-page-items">
                {orderItems.map(item => (
                  <div key={item.id} className="order-page-item">
                    <img src={item.image} alt={item.name} className="order-page-item-image" />
                    <div className="order-page-item-details">
                      <h3>{item.name}</h3>
                      <p className="order-page-item-price">${item.price.toFixed(2)}</p>
                    </div>
                    ...
                    <p className="order-page-item-total">${(item.price * item.quantity).toFixed(2)}</p>
                  </div>
                ))}
              </div>
            ...
            <div className="order-page-payment animate-fade-in">
              ...
              <div className="order-page-payment-methods">
                ...
                <div className="order-page-payment-options">
                  <div 
                    className={`order-page-payment-option ${formData.paymentMethod === 'creditCard' ? 'active' : ''}`}
                  ...
                  <div 
                    className={`order-page-payment-option ${formData.paymentMethod === 'paypal' ? 'active' : ''}`}
                  ...
                  <div 
                    className={`order-page-payment-option ${formData.paymentMethod === 'razorPay' ? 'active' : ''}`}
                  ...
                  <div 
                    className={`order-page-payment-option ${formData.paymentMethod === 'bankTransfer' ? 'active' : ''}`}
                ...
                <div className="order-page-payment-method-content">
                  <div className="order-page-payment-method-info">
                    ...
                    <div className="order-page-payment-method-text">
                      ...
                      <p>Complete your purchase quickly and securely with Razor Pay.</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="order-page-payment-method-content">
                  <div className="order-page-payment-method-info">
                    ...
                    <div className="order-page-payment-method-text">
                      ...
                      <p>Make a direct bank transfer to our account. Order will be processed after payment confirmation.</p>
                    </div>
                  </div>
              ...
              <div className="order-page-final-summary">
                <h3>Order Summary</h3>
                <div className="order-page-summary-row">
                  <span>{orderItems.reduce((total, item) => total + item.quantity, 0)} Items</span>
                  <span>${calculateSubtotal().toFixed(2)}</span>
                </div>
                <div className="order-page-summary-row">
        ...
        <div className="order-page-actions">
          ...
          {activeStep < 3 ? (
            ...
            <PayButton className="order-page-btn primary" amount={250} user={"6815989ec51b03b88e4724b3"} orderId={"6802dd5a2b9c8c1da8ac16ef"}>Place Order</PayButton>
          )}
        </div>
      </div>
    </div>
